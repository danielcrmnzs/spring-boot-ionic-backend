package com.devcoi.cursomc.services.validation.utils;

//Fonte:  * @see {@link https://gist.github.com/adrianoluis/5043397d378ae506d87366abb0ab4e30 }
/**
 * 
 * @author adrianoluis
 * 
...
public class BR {
	...
	private static boolean check(String cpf_cnpj, int length, int[] weight) {
		final String number = cpf_cnpj.substring(0, length);
		final int digit1 = calculate(number, weight);
		final int digit2 = calculate(number + digit1, weight);
		return cpf_cnpj.equals(number + digit1 + digit2);
	}
